 ...
 */
class show_controller extends cars_controller{
	function index_action(){   
		$id=(int)$_GET['id'];
		$M=$this->MODEL('cars');
		$news=$M->GetNewsBaseOnce(array('id'=>$id));
		$row=$M->GetNewsContentOnce(array('nbid'=>$id)); 
        $news_class=$M->GetNewsGroupOnce(array('id'=>$news['nid']));
		...
		if($news["keyword"]!="")
		{
			...
			if(is_array($keyarr) && !empty($keyarr))
			{
				...
				$about=$M->GetNewsBaseList(array("(".$sqlkw.") and `id`<>'".$id."' and `newsphoto`<>''"),array("orderby"=>'`id` desc ','limit'=>3));
				if(is_array($about)){
