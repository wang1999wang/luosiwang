...
class common{
    ...
    function get_home_nav(){        
        //头部导航一级
        $M=$this->MODEL();
        $nav_list1=$M->DB_select_all('navigation','`nid`=1 and `pid`=0 and `display`=1 order by sort asc');
        //头部导航二级
        $currenturl=trim(str_replace('index.php','',$_SERVER['REQUEST_URI']));
        foreach($nav_list1 as $k1=>$v1){
            $PIDList[]=$v1['id'];
            /*switch($currenturl){
                case '':
                case '/':$nav_list1[$k1]['class']='curr';break;
                case strpos($currenturl,trim(str_replace('index.php','',$v1['url']))):$nav_list1[$k1]['class']='curr';break;
            }*/
        }
        
        $nav_list2=$M->DB_select_all('navigation','`nid`=1 and `pid` in ('.implode(',',$PIDList).') order by sort asc');
        foreach($nav_list1 as $k1=>$v1){
            foreach($nav_list2 as $k2=>$v2){
                if($v1['id']==$v2['pid']){
                    $nav_list1[$k1]['sonlist'][]=$v2;
                }
            }
        }
        $this->yunset("nav_list1",$nav_list1);
    }
	...
	function yuntpl($tplarr=array()){
		...
		}else if(!empty($tplarr)){
			$this->tpl->display($tplarr.".htm");
		}else{
			echo "模版不能为空！";die;
		}
	}
	...
	function seo($ident,$title='',$keyword='',$description=''){
		...
		if(is_array($seo)){
			foreach($seo as $k=>$v){
				if($_SESSION['did']!="" && $_SESSION['did']==$v['did']){
					$fzseo=$v;
				}else{
					$seo=$v;
				}
		...
		if($_SESSION['auid'] && $_SESSION['ashell']){
			...
			if($_GET['m']!="admin_right"){
                ...
                if(is_numeric($this->config['did'])){
                    $info=$this->obj->DB_select_once("admin_navigation","`url`='".$url."' and `dids`=1");
                }else{
					$info=$this->obj->DB_select_once("admin_navigation","`url`='".$url."'");
				}
                if(!$info){
