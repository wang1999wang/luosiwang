	...
	foreach($seo as $k=>$v){
		...
		if($v[$i]['rewrite_url'] && $v[$i]['php_url']){
			...
			if($type!=''){
				...
				if($paramer['m']==$urlFileds['m'] && $paramer['c']==$urlFileds['c'] && $paramer['a']==$urlFileds['a']){
...
function get_url($paramer,$config,$seo,$type='',$index='',$_smarty_tpl){
    ...
    if($config['sy_seo_rewrite'] && $index!='admin' && $index!='member' && !$paramer['page'] && !$paramer['keyword'] && $paramer['m']!='ajax'){
        ...
        if(($config['sy_'.$m.'_web']==1)&&(trim($config['sy_'.$m.'dir']))&&(!trim($config['sy_'.$m.'domain']))){
...
function navcalss($menu,$classname){
    ...
    if(!($config['sy_seo_rewrite']=='1' && $val[furl]!='')){
        ...
        $NavAllPath=$config['sy_weburl'].((substr($menu['url'],0,1)=='/')?'':'/').$menu['url'];
        $CurrentAllPath=FetchMCA2NavUrl($CurrentAllPath);
        $NavAllPath=FetchMCA2NavUrl($NavAllPath);
        if(!strstr($NavAllPath,'?')){$NavAllPath.='?c=index';}
        if((strstr($CurrentAllPath,$NavAllPath)!=false)){
            return $classname;
        }else{
            return;
        }
