...
function BackupDatabaseInit($add){
	global $db,$fun_r,$phpyun_db_ver,$config;
	...
	echo "<script>self.location.href='".$config['sy_weburl']."/admin/index.php?m=database&c=$phome&t=0&s=0&p=0&mypath=$add[mypath]&waitbaktime=$waitbaktime';</script>";
	exit();
}
...
function BackupDatabaseFileSize($t,$s,$p,$mypath,$alltotal,$thenof,$fnum,$stime=0){	
	global $db,$bakpath,$limittype,$fun_r,$db_config,$config;
...
?>";
	...
	$p=(int)$p;
	if($t>=$count){
		echo Encode("<script>alert('备份成功！\\n\\n整个过程耗时：".ToChangeUseTime($stime)."');self.location.href='".$config['sy_weburl']."/admin/index.php?m=database';</script>",'gbk');
		die;
    }
	...
	foreach($sql as $k=>$r){
		...
		if(strlen($dumpsql)>=$b_filesize*1024){
			...
			$db->free($sql);
			echo Encode("<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=".$config['sy_weburl']."/admin/index.php?m=database&c=BackupDatabaseFileSize&phome=BakExe&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&stime=$stime&waitbaktime=$waitbaktime&collation=$collation\">".$fun_r['BakOneDataSuccess'],'gbk').EchoBackupProcesser($btb[$t],$count,$t,$num,$s);
			exit();
		}
	}
	if(empty($p)||$b==1){
	...
	$t++;
	$db->free($sql);
	echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=".$config['sy_weburl']."/admin/index.php?m=database&c=BackupDatabaseFileSize&phome=BakExe&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime\">".$fun_r['OneTableBakSuccOne'].$btb[$t-1].$fun_r['OneTableBakSuccTwo'];
	exit();
}
function BackupDatabaseRecordNum($t,$s,$p,$mypath,$alltotal,$thenof,$fnum,$auf='',$aufval=0,$stime=0){
	global $db,$bakpath,$limittype,$fun_r;
	$path=PLUS_PATH.'/bdata/'.$mypath;
	@include($path."/config.php");
	$waitbaktime=(int)$_GET['waitbaktime'];
	if(empty($stime)){
...
?>";
	...
	$p=(int)$p;
	if($t>=$count){
		echo"<script>alert('".$fun_r['BakSuccess']."\\n\\n".$fun_r['TotalUseTime'].ToChangeUseTime($stime)."');self.location.href='".$config['sy_weburl']."/admin/index.php?m=database';</script>";
		exit();
    }
	$u=$db->query("use `$b_dbname`");
	if($b_dbchar=='auto'){
	...
	if(empty($b)){
		...
		$t++;
		$db->free($sql);
		echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=".$config['sy_weburl']."/admin/index.php?m=database&c=BackupDatabaseRecordNum&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime\">".$fun_r['OneTableBakSuccOne'].$btb[$t-1].$fun_r['OneTableBakSuccTwo'];
		exit();
	}
	$p++;
	$sfile=$path."/".$btb[$t]."_".$p.".php";
