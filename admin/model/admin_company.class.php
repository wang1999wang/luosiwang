...
class admin_company_controller extends common
{
	...
	function getrating_action(){
		...
		if($_POST['id'])
		{
			$rating	= $this->obj->DB_select_once("company_rating","`id`='".intval($_POST['id'])."'","`resume`,`job_num`,`interview`,`editjob_num`,`breakjob_num`,`lt_job_num`,`lt_resume`,`lt_editjob_num`,`lt_breakjob_num`,`msg_num`,`service_time`,`coupon`,`category`,`type`");
	...
	function uprating_action(){
		 ...
		 if($_POST['ratuid']){
			...
			$id?$this->ACT_layer_msg("企业会员等级(ID:".$aid.")修改成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg("修改失败！",8,$_SERVER['HTTP_REFERER']);
		}else{
	    ...
	    if($_GET['del']){
	    	...
	    	if($del){
	    		if(is_array($del)){
	    			...
	    			$uids = @implode(",",$del);
	    			foreach($del as $k=>$v){
	    				$this->obj->delfiledir("../data/upload/tel/".intval($v));
	    			}
		    		...
		    		$this->obj->delfiledir("../data/upload/tel/".$del);
	...
	function status_action(){
		 ...
		 if(is_array($member)&&$member){
			...
			$id?$this->ACT_layer_msg("企业会员审核(ID:".$aid.")设置成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg("审核设置失败！",8,$_SERVER['HTTP_REFERER']);
		}else{
			$this->ACT_layer_msg( "非法操作！",8,$_SERVER['HTTP_REFERER']);
		}
	}
