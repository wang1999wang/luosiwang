...
class user_member_controller extends common
{
	...
	function index_action(){
		...
		if($_GET['source']){
			$where .=" and b.`source` = '".$_GET['source']."'";
			$page_url['source']=$_GET['source'];
		}
		if($_GET['r_uid'])//举报页传递参数
		{
		...
		$count=$this->obj->DB_select_alls("resume","member","a.`uid`=b.`uid` ".$where,"a.uid");//获取总数
 		$num = count($count);
	    	...
	    	if($del){
	    		if(is_array($del)){
	    			foreach($del as $k=>$v){
	    				...
	    				$this->obj->delfiledir("../data/data/upload/tel/".intval($v));
	    			}
		    		$uids = @implode(",",$del);
					...
					$del_array=array("member","resume","member_statis","look_resume","look_job","resume_show","userid_msg","userid_job","resume_expect","resume_cert","resume_edu","resume_other","resume_project","resume_skill","resume_training","resume_work","resume_doc","user_resume","resume_show","down_resume","userid_job","friend_info","friend_message","friend_state","question","msg","attention","rebates","company_msg","px_subject_collect","px_zixun","down_reume");
