...
class admin_resume_controller extends common
{
	...
	function index_action(){
        ...
        $M=$this->MODEL();
		$PageInfo=$M->get_page("resume_expect",$where,$pageurl,$this->config['sy_listnum']);
	...
	function recommend_action(){
		if($_GET['type']){
			...
			if($nid){
				$this->obj->admin_log("简历置顶(ID:".$eid.")");
	...
	function xls_action()
	{
		if($_POST['where'])
		{
			...
			if(!empty($list))
			{
				...
				$CacheArr['user'] =array('userclass_name');
				$this->CacheInclude($CacheArr);
