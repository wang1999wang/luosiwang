 ...
 */
class admin_cars_controller extends common{
	function set_search(){
	...
	function property_action(){
		if(!$_POST['id']){
			...
			isset($nbid)?$this->ACT_layer_msg( "叉车属性(ID:".$nbid.")添加成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg( "添加失败！",8,$_SERVER['HTTP_REFERER']);
		}else{
			$nbid=$this->obj->DB_update_all("property","`name`='".$_POST['name']."',`value`='".$_POST['value']."'","`id`='".$_POST['id']."'");
			isset($nbid)?$this->ACT_layer_msg( "叉车属性(ID:".$_POST['id'].")更新成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg( "更新失败！",8,$_SERVER['HTTP_REFERER']);
		}
	}
	function savepro_action(){
		if($_POST['type']=="add"){
			...
			$this->ACT_layer_msg("叉车(ID:".$_POST['proid'].")添加属性成功！",9,$_SERVER['HTTP_REFERER'],2,1);
		}
		if($_POST['type']=="del" && $_POST['proid']){
			...
			$this->ACT_layer_msg("叉车(ID:".$_POST['proid'].")删除属性成功！",9,$_SERVER['HTTP_REFERER'],2,1);
		}
	}
	function delpro_action(){
		...
		if($_GET['id']){
			...
			$this->layer_msg("叉车属性(ID:".$_POST['id'].")删除成功",9,0,$_SERVER['HTTP_REFERER']);
		}
	}
	function news_action(){
		...
		if($_GET['id']){
            ...
            $picfiles_list=json_decode($news['pic_files'],true);
			foreach($picfiles_list as $k1=>$v1){
				$picfiles_list_new[]=array(urldecode($v1[0]),urldecode($v1[1]),urldecode($v1[2]),urldecode($v1[3]));
			}
			$this->yunset("picfiles_list",$picfiles_list_new);
            
			$params_list=json_decode($news['params'],true);
			foreach($params_list as $k1=>$v1){
				$params_list_new[]=array(urldecode($v1[0]),urldecode($v1[1]),urldecode($v1[2]));
			}
            $news['gongzuochangdi']=explode(',',$news['gongzuochangdi']);
			$this->yunset("params_list",$params_list_new);
            $this->yunset("nid_list",explode(',',$news['nid']));
	...
	function addcars_action()
	{
		if($_POST['updatenews'])
		{
            ...
            $data['nid']=implode(',',$_POST['nid']);
            $data['lastupdate']=$time;
            $data['describe']=$describe;
            $data['content']=$content;
            //叉车图片
			foreach($_POST['pic_code'] as $k1=>$v1){
				$pic_files[]=array(urlencode($_POST['pic_title'][$k1]),urlencode($v1),urlencode($_POST['pic_desc'][$k1]),$_POST['pic_file'][$k1]);
			}
            $data['pic_files']=json_encode($pic_files);
			//叉车参数
			foreach($_POST['param_key'] as $k1=>$v1){
				$params[$_POST['param_sort'][$k1]]=array(urlencode($v1),urlencode($_POST['param_value'][$k1]),$_POST['param_sort'][$k1]);
			}
            ksort($params);
            ...
            $data['gongzuochangdi']=implode(',',$_POST['gongzuochangdi']);
            ...
            $url=Url('admin_cars',null,'admin');
			isset($nbid)?$this->ACT_layer_msg( "叉车(ID:".$_POST['id'].")更新成功！",9,Url('admin_cars',null,'admin'),2,1):$$this->ACT_layer_msg( "更新失败！",8,Url('admin_cars',null,'admin'));
		}
	    if($_POST['newsadd']){
            $data=$_POST;
			if($_POST['uplocadpic']){
			...
			isset($nbid)?$this->ACT_layer_msg( "叉车(ID:".$nbid.")添加成功！",9,Url('admin_cars',null,'admin'),2,1):$this->ACT_layer_msg( "添加失败！",8,Url('admin_cars',null,'admin'));
		}
	}
	...
	function addgroup_action(){
        if($_POST['sub']){
			if($_POST['classname']!=""){
				if(!is_array($this->obj->DB_select_once("cars_group","name='".$_POST['classname']."'"))){
					...
					$this->get_cache();
					isset($nbid)?$this->ACT_layer_msg( "叉车类别(ID:".$nbid.")添加成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg( "添加失败！",8,$_SERVER['HTTP_REFERER']);
			     }else{
	    ...
	    if($_POST['update']){
	         ...
	         $this->get_cache();
			 isset($update)?$this->ACT_layer_msg( "叉车类别(ID:".$_POST['id'].")更新成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg( "更新失败！",8,$_SERVER['HTTP_REFERER']);
	    }
	}
	    ...
	    if($_GET['del']){
	    	...
	    	if(is_array($del)){
				...
				$this->obj->DB_delete_all("news_content","`nbid` in(".@implode(',',$del).")","");
 				$this->layer_msg('叉车(ID:'.@implode(',',$del).')删除成功！',9,1,$_SERVER['HTTP_REFERER']);
	    	}else{
				$this->layer_msg('请选择您要删除的叉车！',8,1,$_SERVER['HTTP_REFERER']);
	    	}
	    }
	    if(isset($_GET['id'])){
			$where="`id`='".$_GET['id']."'";
			...
			isset($nid)?$this->layer_msg('叉车(ID:'.$_GET['id'].')删除成功！',9):$this->layer_msg('删除失败！',8);
		}else{
	...
	function delgroup_action()
	{
	   ...
	   if(isset($_GET['id']))
	   {
			...
			$this->get_cache();
			isset($result)?$this->layer_msg('叉车类别(ID:'.$_GET['id'].')删除成功！',9):$this->layer_msg('删除失败！',8);
	   }
	}
	...
	function ajax_action()
	{
		if($_POST['sort'])
		{
			...
			$this->admin_log("叉车类别(ID:".$_POST['id'].")修改排序");
		}
		if($_POST['name'])
		{
			...
			$this->admin_log("叉车类别(ID:".$_POST['id'].")修改名称");
		}
		$this->get_cache();
		echo '1';die;
	}
	...
	function delmenu_action()
	{
		if($_GET['id'])
		{
			...
			$this->menu_cache_action();
			$this->layer_msg("叉车类别导航(".$_GET['id'].")取消成功",9,0,$_SERVER['HTTP_REFERER']);
		}else{
